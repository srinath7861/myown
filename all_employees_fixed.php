        ...
        <section id="admin-dashboard" class="page-section py-4 sm:py-6">
            <div class="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6">
                ...
                <div class="bg-white rounded-lg shadow-sm border border-gray-200 card-hover mb-6">
                    ...
                    <div class="p-4 sm:p-6">
                        ...
                        <div class="grid grid-cols-7 gap-1">
                            ...
                            ?>
                                <div class="bg-white p-2 h-20 sm:h-24"></div>
                            <?php
                            endfor;
                            ...
                            ?>
                                <button 
                                    ...
                                    class="calendar-day bg-white p-2 h-20 sm:h-24 border border-gray-200 flex flex-col rounded-md text-left focus:outline-none focus:ring-2 focus:ring-blue-500 <?php echo $is_selected ? 'border-blue-500 border-2' : ''; ?>">
                ...
                <div x-show="selectedDate" class="bg-white rounded-lg shadow-sm border border-gray-200 card-hover" x-cloak>
                    <div class="px-4 sm:px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                        <h3 class="text-lg font-medium text-gray-900">Attendance Details for <span x-text="formatDate(selectedDate)"></span></h3>
                ...
                <div x-show="detailsModalOpen" 
                     ...
                     x-cloak>
                    <div @click.stop 
                         class="bg-white rounded-lg p-4 sm:p-6 max-w-sm sm:max-w-md w-full modal-content">
                        <h2 class="text-lg font-medium text-gray-900 mb-4" id="modal-title">
                            Attendance Summary for <span x-text="formatDate(modalDate)"></span>
                        </h2>
                        <template x-if="modalRecords.length > 0">
                            <div class="space-y-4">
                                <div class="grid grid-cols-2 gap-4">
                                    <div>
                                        <p class="text-sm font-medium text-gray-700">Total Employees</p>
                                        <p class="text-sm text-gray-900" x-text="modalRecords.length"></p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-gray-700">Present</p>
                                        <p class="text-sm text-gray-900" x-text="modalRecords.filter(r => r.status.includes('Present') || r.status.includes('Half Day')).length"></p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-gray-700">Absent</p>
                                        <p class="text-sm text-gray-900" x-text="modalRecords.filter(r => r.status.includes('Absent')).length"></p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-gray-700">Late Arrivals</p>
                                        <p class="text-sm text-gray-900" x-text="modalRecords.filter(r => r.status.includes('Late')).length"></p>
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-gray-700">Half Days</p>
                                        <p class="text-sm text-gray-900" x-text="modalRecords.filter(r => r.status.includes('Half Day')).length"></p>
                                    </div>
                                </div>
                            </div>
                        </template>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('allEmployees', () => ({
                ...
                openDetails(date) {
                    // Store the date for the modal separately
                    this.modalDate = date;
                    this.selectedDate = date;
                    this.currentPage = 1;
                    this.searchQuery = '';
                    
                    // Make a copy of filtered records for the modal
                    this.modalRecords = [...this.filteredRecords];
                ...
                closeDetailsModal() {
                    ...
                    // Don't clear modalDate here to preserve it for display
                },
                ...
                formatTime(time) {
                    if (!time) return '-';
                    const date = new Date('2000-01-01 ' + time.substring(11));
